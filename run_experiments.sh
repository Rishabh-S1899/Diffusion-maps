...
# 2. CACHING RUN
echo -e "\n[2/3] RUNNING CACHING..."
uv run python demo-sd3-5.py --prompts $PROMPTS --seed $SEED --profile --cache_schedule "$SCHEDULE" --output_dir "$BASE_OUT/cached"

# 3. CACHING + QUANTIZATION RUN
echo -e "\n[3/3] RUNNING CACHING + 8-BIT QUANTIZATION..."
uv run python demo-sd3-5.py --prompts $PROMPTS --seed $SEED --profile --cache_schedule "$SCHEDULE" --quantize_cache --output_dir "$BASE_OUT/cached_quantized"
